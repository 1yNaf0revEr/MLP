# Fashion-MNIST MLP Classifier (NumPy 手工实现)
纯 NumPy 从零实现三层 MLP 分类器，完成 Fashion-MNIST 服装 10 分类任务。
全程不依赖深度学习框架，手动实现前向传播、反向传播、优化器、正则化与完整评估可视化。

## 环境配置
Python 3.12
依赖：numpy>=1.26、matplotlib>=3.8
...
## 模型结构
标准三层 MLP：
Input(784) → Linear → Activation → Linear → Activation → Linear(10)

默认结构：784 → 256 → 128 → 10

## 运行方式
一键启动完整流程（数据→搜索→训练→测试→可视化）：
python main.py

加速调试：在 main.py 中缩小超参数搜索空间即可。
